from DataStructureModule import PrintTable
import json
import os
import pathlib
...
# 编译预处理，取出无用的字符和注释
def filterResource(r: str, pProject):
    tempString = []
    count = 0
    i = 0
    while i < pProject:
...
def parse_sql(resourceProject: str) -> list:
    from DataStructureModule import PrintTable
    ...
    while syn != 0:
        ...
        if syn == 100:
            ...
            if len(result[-1]) == 1:
                ...
                if (
                    result[-1][1][-1] not in operatorOrDelimiter
                    and getStr(token) not in operatorOrDelimiter
                ):
                    result[-1][1] += " "
...
if __name__ == "__main__":
    parse_sql("select a.No from Student a where a.no = 1")